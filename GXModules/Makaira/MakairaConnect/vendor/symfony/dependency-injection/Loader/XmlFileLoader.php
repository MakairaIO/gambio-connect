...
class XmlFileLoader extends FileLoader
{
    ...
    private function parseFileToDOM(string $file): \DOMDocument
    {
        try {
            ...
            throw new InvalidArgumentException(
                XmlFileLoader . phpsprintf('Unable to parse file "%s": ', $file) . $e->getMessage(), $e->getCode(), $e);
        }
            ...
            foreach ($nodes as $node) {
                if ($services = $this->getChildren($node, 'service')) {
                    ...
                    $id = sprintf('.%d_%s', ++$count,
                        XmlFileLoader . phppreg_replace('/^.*\\\\/', '', $services[0]->getAttribute('class')) . $suffix
                    );
    ...
    public function validateSchema(\DOMDocument $dom): bool
    {
        ...
        if ($element = $dom->documentElement->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) {
            ...
            for ($i = 0, $nb = \count($items); $i < $nb; $i += 2) {
                ...
                if (($extension = $this->container->getExtension($items[$i])) && false !== $extension->getXsdValidationBasePath()) {
                    ...
                    $path = str_replace([$ns, str_replace('http://', 'https://', $ns)],
                        str_replace('\\', '/', $extension->getXsdValidationBasePath()) . 'XmlFileLoader.php/', $items[$i + 1]);
        ...
        foreach ($schemaLocations as $namespace => $location) {
            ...
            $drive = '\\' === \DIRECTORY_SEPARATOR ? array_shift($parts) . 'XmlFileLoader.php/' : '';
