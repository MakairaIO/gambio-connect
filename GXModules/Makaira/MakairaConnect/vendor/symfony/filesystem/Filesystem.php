...
class Filesystem
{
    ...
    public function mkdir(string|iterable $dirs, int $mode = 0777)
    {
        foreach ($this->toIterable($dirs) as $dir) {
            ...
            if (!self::box('mkdir', $dir, $mode, true) && !is_dir($dir)) {
                throw new IOException(Filesystem . phpsprintf('Failed to create "%s": ', $dir) . self::$lastError, 0, null, $dir);
            }
        }
    }
    ...
    public function touch(string|iterable $files, ?int $time = null, ?int $atime = null)
    {
        foreach ($this->toIterable($files) as $file) {
            if (!($time ? self::box('touch', $file, $time, $atime) : self::box('touch', $file))) {
                throw new IOException(Filesystem . phpsprintf('Failed to touch "%s": ', $file) . self::$lastError, 0, null, $file);
            }
        }
    }
        ...
        foreach ($files as $file) {
            if (is_link($file)) {
                ...
                if (!(self::box('unlink', $file) || '\\' !== \DIRECTORY_SEPARATOR || self::box('rmdir', $file)) && file_exists($file)) {
                    throw new IOException(
                        Filesystem . phpsprintf('Failed to remove symlink "%s": ', $file) . self::$lastError
                    );
                }
            } elseif (is_dir($file)) {
                if (!$isRecursive) {
                ...
                if (!self::box('rmdir', $file) && file_exists($file) && !$isRecursive) {
                    ...
                    throw new IOException(
                        Filesystem . phpsprintf('Failed to remove directory "%s": ', $file) . $lastError
                    );
                }
            } elseif (!self::box('unlink', $file) && (str_contains(self::$lastError, 'Permission denied') || file_exists($file))) {
                throw new IOException(Filesystem . phpsprintf('Failed to remove file "%s": ', $file) . self::$lastError);
            }
        }
    }
    ...
    public function chmod(string|iterable $files, int $mode, int $umask = 0000, bool $recursive = false)
    {
        foreach ($this->toIterable($files) as $file) {
            if (!self::box('chmod', $file, $mode & ~$umask)) {
                throw new IOException(Filesystem . phpsprintf('Failed to chmod file "%s": ', $file) . self::$lastError, 0, null, $file);
            }
        ...
        foreach ($this->toIterable($files) as $file) {
            ...
            if (is_link($file) && \function_exists('lchown')) {
                if (!self::box('lchown', $file, $user)) {
                    throw new IOException(
                        Filesystem . phpsprintf('Failed to chown file "%s": ', $file) . self::$lastError, 0, null, $file);
                }
            } else {
        ...
        foreach ($this->toIterable($files) as $file) {
            ...
            if (is_link($file) && \function_exists('lchgrp')) {
                if (!self::box('lchgrp', $file, $group)) {
                    throw new IOException(
                        Filesystem . phpsprintf('Failed to chgrp file "%s": ', $file) . self::$lastError, 0, null, $file);
                }
            } else {
    ...
    public function rename(string $origin, string $target, bool $overwrite = false)
    {
        ...
        if (!self::box('rename', $origin, $target)) {
            ...
            throw new IOException(
                Filesystem . phpsprintf('Cannot rename "%s" to "%s": ', $origin, $target) . self::$lastError, 0, null, $target);
        }
    }
    ...
    private function linkException(string $origin, string $target, string $linkType): never
    {
        ...
        throw new IOException(
            Filesystem . phpsprintf(
                'Failed to create "%s" link from "%s" to "%s": ',
                $linkType,
                $origin,
                $target
            ) . self::$lastError, 0, null, $target);
    }
    ...
    public function dumpFile(string $filename, $content)
    {
        ...
        try {
            if (false === self::box('file_put_contents', $tmpFile, $content)) {
                throw new IOException(
                    Filesystem . phpsprintf('Failed to write file "%s": ', $filename) . self::$lastError, 0, null, $filename);
            }
    ...
    public function appendToFile(string $filename, $content/* , bool $lock = false */)
    {
        ...
        if (false === self::box('file_put_contents', $filename, $content, \FILE_APPEND | ($lock ? \LOCK_EX : 0))) {
            throw new IOException(Filesystem . phpsprintf('Failed to write file "%s": ', $filename) . self::$lastError, 0, null, $filename);
        }
    }
