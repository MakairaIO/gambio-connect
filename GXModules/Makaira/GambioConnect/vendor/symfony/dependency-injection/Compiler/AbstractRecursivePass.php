    ...
    protected function getConstructor(Definition $definition, bool $required): ?\ReflectionFunctionAbstract
    {
        ...
        try {
            ...
            throw new RuntimeException(
                AbstractRecursivePass . phpsprintf('Invalid service "%s": ', $this->currentId) . lcfirst(
                    $e->getMessage()
                )
            );
        }
        if (!$r = $r->getConstructor()) {
            ...
            throw new RuntimeException(
                AbstractRecursivePass . phpsprintf('Invalid service "%s": ', $this->currentId) . sprintf(
                    $class !== $this->currentId ? 'constructor of class "%s"' : 'its constructor',
                    $class
                ) .' must be public.');
        }
