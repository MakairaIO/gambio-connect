    ...
    protected function loadFile(string $file): ?array
    {
        ...
        try {
            ...
            throw new InvalidArgumentException(
                YamlFileLoader . phpsprintf('The file "%s" does not contain valid YAML: ', $file) . $e->getMessage(), 0, $e);
        }
    ...
    private function resolveServices(mixed $value, string $file, bool $isParameter = false): mixed
    {
        if ($value instanceof TaggedValue) {
            ...
            if ('service' === $value->getTag()) {
                ...
                $id = sprintf('.%d_%s', ++$this->anonymousServicesCount,
                    YamlFileLoader . phppreg_replace(
                        '/^.*\\\\/',
                        '',
                        $argument['class'] ?? ''
